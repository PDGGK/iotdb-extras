...
it runs in the `integration-test` phase and requires Docker. Run just the
benchmark with the `benchmark` tag group:

```bash
mvn -o -Dmaven.repo.local=<repo> integration-test -Dgroups=benchmark
```

Use `-Dgroups=benchmark`, **not** `-Dtest=IoTDBTableIngestionBenchmarkIT`: a
global `-Dtest=` overrides the include/exclude filters of *both* surefire
executions, which would pull the Docker IT into the unit `test` phase (and then
run it again in `integration-test`). The tag filter is applied on top of the
file patterns instead, so the unit execution still excludes `**/*IT.java` and
only the `integration-tests` execution runs the tagged benchmark.

Or run the whole integration-test phase (all `*IT.java`):
