 ...
 * historical READ path, the millisecond time-bucketed aggregation READ path ({@link #findAllAsync})
 * and the DELETE path ({@link #remove}), all driven through a bounded read thread-pool. Aggregation
 * walks fixed-width {@code date_bin} buckets anchored at the query start timestamp; week, month and
 * quarter bucket boundaries are a later increment.
 */
@Slf4j
@Repository
...
public class IoTDBTableTimeseriesDao extends IoTDBTableBaseDao
  ...
  private ReadTsKvQueryResult readAggregatedQuery(
      TenantId tenantId, EntityId entityId, ReadTsKvQuery query) throws Exception {
    // Calendar-interval (WEEK/WEEK_ISO/MONTH/QUARTER) buckets are timezone-aware and
    // calendar-aligned (variable width), which the fixed-width millisecond date_bin path below
    // cannot reproduce. AggregationParams maps those interval types to a fixed millisecond span
    // (e.g. MONTH -> 30 days), so without this guard a calendar query would reach getInterval() >=
    // 1
    // and be silently bucketed as fixed-width — a wrong answer. Fail loud instead; calendar-bucket
    // aggregation is a later increment.
    IntervalType intervalType = query.getAggParameters().getIntervalType();
    if (intervalType != null && intervalType != IntervalType.MILLISECONDS) {
      throw new UnsupportedOperationException(
          "Calendar-interval (WEEK/WEEK_ISO/MONTH/QUARTER) aggregation is not yet supported by this "
              + "IoTDB Table Mode backend; only millisecond-interval aggregation is available.");
    }
  ...
  private String buildAggregationSql(
    ...
    // a
    // zero-width [startTs, startTs] query still walks the single [startTs, startTs+1) bucket and
  ...
  private static String aggregationProjection(Aggregation aggregation) {
    return switch (aggregation) {
      case AVG -> "AVG(" + NUMERIC_VALUE + ") AS " + AGG_NUM_COLUMN;
        ...
        // counts
        // so the row mapper can pick the type without re-reading the raw rows.
      case SUM ->
          ...
          // out
          // of range of long value" error at the IoTDB level when the long-only sum exceeds
    ...
    return switch (aggregation) {
      case AVG -> {
        ...
        if (row.isNull(AGG_NUM_COLUMN)) {
          // Non-empty bucket with no numeric rows (only boolean/string/json). date_bin never emits
          // a
          // truly-empty bucket, so this row IS a non-empty bucket; ThingsBoard 4.3.1.2 findAvg
          // reports AVG=0.0 (the totalCount==0 else-branch) here rather than dropping the bucket.
          yield new DoubleDataEntry(key, 0.0);
        }
   ...
   * values and accumulate them in Java as {@code long} (deterministic 2's-complement wrap should
   * the total ever exceed 2^63 -- an astronomically-rare edge). The fast SQL path handles every
   * normal bucket; only genuinely huge buckets re-query.
   */
  private KvEntry aggregatedSumEntry(SessionDataSet.DataIterator row, SumReSumContext sumContext)
    ...
    long countDouble = countColumn(row, COUNT_DOUBLE_COLUMN);
    if (countLong == 0L && countDouble == 0L) {
      // Non-empty bucket with no numeric rows (only boolean/string/json). date_bin never emits a
      // truly-empty bucket, so this row IS a non-empty bucket; ThingsBoard 4.3.1.2 findSum reports
      // SUM=0 (LONG-typed, the doubleCount==0 branch) here rather than dropping the bucket.
      return new LongDataEntry(key, 0L);
    }
   ...
   * {@code long}, with deterministic 2's-complement wrap should the total ever exceed 2^63. Used
   * only when the bucket's values are large enough that IoTDB's DOUBLE SUM accumulator may have
   * lost precision (see {@link #sumIsProvablyExact}); the bounded window {@code [bucketStart,
   ...
   * matters for multi-typed (Phase-1 stale) buckets.
   */
